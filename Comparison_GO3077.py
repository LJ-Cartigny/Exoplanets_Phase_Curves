...
def main():
    ...
    visit12_flux_obs = load_data("Data_GO_3077/S3_miri_photometry_T1bc_ap10_bg15_45_SpecData_"+visit+".h5", 'aplev')
    visit12_flux_err_obs = load_data("Data_GO_3077/S3_miri_photometry_T1bc_ap10_bg15_45_SpecData_"+visit+".h5", 'aperr')
    visit12_time_obs = load_data("Data_GO_3077/S3_miri_photometry_T1bc_ap10_bg15_45_SpecData_"+visit+".h5", 'time') -50000
    ...
    plt.errorbar(visit12_time_obs, visit12_flux_obs, yerr=visit12_flux_err_obs, fmt='o', label='Visit 1 & 2 observations', color='green', markersize=2, capsize=3)

    plt.plot(visit1_time_sphinx, visit1_flux_sphinx, label='Visit 1 simulation (SPHINX)', color='blue', linestyle='--')
    plt.plot(visit2_time_sphinx, visit2_flux_sphinx, label='Visit 2 simulation (SPHINX)', color='orange', linestyle='--')
    plt.plot(visit1_time_phoenix, visit1_flux_phoenix, label='Visit 1 simulation (PHOENIX)', color='blue', linestyle='-')
    plt.plot(visit2_time_phoenix, visit2_flux_phoenix, label='Visit 2 simulation (PHOENIX)', color='orange', linestyle='-')

    plt.title('GO 3077 Comparison')
    plt.xlabel(r'Time ($BMJD_{TBD}$)')
    plt.ylabel('Flux (mJy)')
    plt.grid()
    plt.legend()
    plt.xlim(np.min(visit12_time_obs), np.max(visit12_time_obs))
    # plt.ylim(3.625,3.775)
    ax = plt.gca()
    ax.xaxis.set_major_formatter(plt.FormatStrFormatter('%.1f'))

    # plt.savefig('GO_3077_comparison_photutils.png', bbox_inches='tight')
    plt.show()
